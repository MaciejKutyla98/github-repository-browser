...
export const RepositoryDetailsCard = (props) => {
    ...
    const {
        ...
        error: userError,
        data: userData,
    } = useQuery(VIEWER_LOGIN);
    const { loading: repositoryDetailsLoading, data: repositoryDetailsData } =
        useQuery(REPOSITORY_DETAILS, {
            variables: { name: name },
        });
    const handleChange = (event, newValue) => {
        setValue(newValue);
    };
    const findRepoByName = repositoryDetailsData?.viewer.repositories.nodes.find(
        (node) => node.name === name
    );
    ...
    return repositoryDetailsLoading ? (
        ...
        <Card sx={{ minWidth: 275, margin: 8 }}>
            <CardContent>
                ...
                <Typography
                    sx={{ margin: 2, padding: 2, fontSize: 16 }}
                    variant="body2"
                >
                    Total commits:{" "}
                    {
                        repositoryDetailsData?.viewer?.repository?.object?.history
                            ?.totalCount
                    }
                    <br />
                ...
                <TabPanel value={value} index={0}>
                    {repositoryDetailsData?.viewer.repository.object.history.totalCount ==
                    0 ? (
                        "There is nothing here"
                    ) : (
                        <List>
                            {repositoryDetailsData?.viewer.repository.object.history.nodes.map(
                                (v) => {
                                    return (
                ...
                <TabPanel value={value} index={1}>
                    {findRepoByName.issues.totalCount == 0 ? (
                        "There is nothing here"
                    ) : (
                        <List>
                ...
                <TabPanel value={value} index={2}>
                    {findRepoByName.releases.totalCount == 0 ? (
                        "There is nothing here"
                    ) : (
                        <List>
